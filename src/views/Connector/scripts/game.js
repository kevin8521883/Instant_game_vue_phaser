...
import { Menu, check_audio, switch_audio } from "./menu";
...
class Game extends Phaser.Scene {
  ...
  create() {
    ...
    let txt_coin = this.add
      .text(320, 50, String(game_data.coin), {
    ...
    // let txt_lvl =
    this.add
      .text(485, 50, "Lv." + game_data.cur_level, {
        fontFamily: "robotomono",
        fontSize: 31,
        align: "right",
        color: "#fff",
      ...
      .setOrigin(1, 0.5);
    //
    if (first_play && game_data.cur_level === 1) {
      //Show guide
      state = "guide";
    ...
    this.input.on(
      ...
      function(pointer, obj) {
        if (
          state === "play" &&
          obj.tile &&
          level_data.array[obj.pos.y][obj.pos.x][1] != 3
        ) {
          play_sound("pop", self);
          obj.rotate += 90;
          if (obj.rotate >= 360) {
            ...
            onComplete: function() {
              ...
              if (obj.name === "next") {
                if (game_data.cur_level <= game_data.total_level) {
                  ...
                  alert(
                    "Awesome! You've completed all levels. Level will be reset to 1."
                  );
    ...
    function shuffle() {
      ...
      for (let i = 0; i < total; i++) {
        let p = child[i];
        if (level_data.array[p.pos.y][p.pos.x][1] != 3) {
          //p.type != 8 && p.type != 12
    ...
    function completed() {
      ...
      function show_window() {
        ...
        self.add
          ...
          .setOrigin(0.5);
        self.add
          .text(360, 460, "Bonus coin: +" + arr.length, {
            fontFamily: "robotomono",
            fontSize: 35,
            align: "center",
            color: "#fff",
    ...
    function window_hint() {
      ...
      let caption = self.add
        .text(360, 380, "Show hint", {
          fontFamily: "robotomono",
          fontSize: 50,
          align: "center",
          color: "#fff",
      ...
      let price = self.add
        .text(b_pay.x + 50, b_pay.y, String(game_data.hint_cost), {
          fontFamily: "robotomono",
          fontSize: 60,
          align: "center",
          color: "#fff",
    ...
    function show_hint() {
      ...
      if (qualify.length > 0) {
        setTimeout(function() {
          ...
          picked.rotate = get_angle(
            Number(level_data.array[picked.pos.y][picked.pos.x][0])
          );
          array[picked.pos.y][picked.pos.x].type = get_type(
            picked.type,
            picked.rotate
          );
...
export { config, game_data, draw_button, play_sound, launch };
